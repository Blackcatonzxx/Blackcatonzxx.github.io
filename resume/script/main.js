...
function shiftDownRobbyContainer() {
    ...
    $(robbyContainerDiv).stop().animate({bottom: "20%"}, 500, function () {
        setRobbyStaticFrame();
...
function robbyJumpDown(i) {
    ...
    if ((pageVerticalPosition > elevationArray[i].offsetLeft - robbyRightEdge) && (pageVerticalPosition < elevationArray[i].offsetLeft + elevationArray[i].offsetWidth - robbyLeftEdge)) {
        $(robbyContainerDiv).stop().animate({bottom: [containerDiv.offsetHeight - elevationArray[i].offsetTop, 'easeInCubic']}, 300, function () {
            disableIsRobbyJumpingAndFalling();
            setRobbyStaticFrame()
        });
        setRobbyJumpDownAndFallFrame();
...
function robbyFall(i) {
    ...
    if (((previousPageVerticalPosition < elevationArray[i].offsetLeft + elevationArray[i].offsetWidth - robbyLeftEdge) && (pageVerticalPosition >= elevationArray[i].offsetLeft + elevationArray[i].offsetWidth - robbyLeftEdge))
        ...
        isRobbyFalling = true;
        setRobbyJumpDownAndFallFrame();
        $(robbyContainerDiv).stop().animate({bottom: [containerDiv.offsetHeight - groundAndGrassContainer1Div.offsetTop, 'easeInCubic']}, 300, function () {
            disableIsRobbyJumpingAndFalling();
            setRobbyStaticFrame()
        });
    }
...
function robbySwimUp() {
    ...
    if (swimUpHeight > 0) {
        ...
        $(robbyContainerDiv).stop().animate({bottom: swimUpDistance}, robbySwimUpTime, function () {
...
function robbySwimDown(robbySwimDownTime) {
    $(robbyContainerDiv).stop().animate({bottom: seaFloorDiv.offsetHeight + "px"}, robbySwimDownTime, function () {
        setRobbyStaticFrame()
    });
...
function animateBuildingsEyes() {
    ...
    buildingBlinkTimer = setInterval(function () {
        blinkBuildings()
    }, 4000); //start to blink building eyes
}
...
function nbaBoardsJump() {
    for (var i = 0; i < nbaBoardArray.length; i++) {
        $(nbaBoardArray[i]).stop().delay(i * 100).animate({bottom: [200, 'easeOutCubic']}, 300, function () {
            nbaBoardsFall()
        });
    }
}
...
function nbaPlayerRun() {
    ...
    $(nbaPlayerContainerDiv).stop().animate({left: "690px"}, 1000, function () {
        nbaPlayerJump()
    });
}
...
function nbaPlayerJump() {
    ...
    $(nbaPlayerContainerDiv).stop().animate({left: "570px", bottom: [200, 'easeOutCubic']}, 300, function () {
        nbaPlayerFall()
    });
}
...
function animateSeaAnimals(seaAnimalArray) {
    ...
    for (var i = 0; i < seaAnimalLocalArray.length; i++) {
        $(seaAnimalLocalArray[i]).stop().delay(i * 100).animate({left: [seaAnimalLocalArray[i].offsetLeft - seaAnimalSwimDistance, 'easeOutCubic']}, 600, function () {
...
function animateRobot() {
    $(robotDiv).stop().animate({left: "380px"}, 1000, function () {
...
function animateSquid() {
    $(squidDiv).stop().animate({left: "430px"}, 1000, function () {
...
function animateAlien() {
    $(alienDiv).stop().animate({left: "500px"}, 1000, function () {
...
function animatePiechartIncognitoFront() {
    if ((browserName == "internet explorer") && (browserVersion <= 8)) {
        ...
        $(piechartIncognitoFrontDiv).stop().animate({opacity: 1}, 500, function () {
...
function animatePiechartFoxnewsFront() {
    if ((browserName == "internet explorer") && (browserVersion <= 8)) {
        ...
        $(piechartFoxnewsFrontDiv).stop().animate({opacity: 1}, 500, function () {
...
function animateBubble() {
    ...
    $(bubbleDiv).stop().animate({top: "0px"}, 2 * robbyToSeaSurfaceDistance, function () {
        hideBubble()
    });
}
...
function robbyHandsUp() {
    ...
    setTimeout(function () {
        setRobbyStaticFrame()
    }, 1000); //robby hands down after 1 sec
}
